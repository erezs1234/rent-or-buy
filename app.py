...
def sync_to_activetrail(name, phone, email):
    """Sync lead data to ActiveTrail API"""
    headers = {"Authorization": ACTIVETRAIL_API_KEY}
    
    try:
        # 1. Find Group ID by Name
        group_id = None
        groups_resp = requests.get("https://api.activetrail.com/v1/groups", headers=headers)
        if groups_resp.status_code == 200:
            groups = groups_resp.json()
            for g in groups:
                if g['name'] == ACTIVETRAIL_GROUP_NAME:
                    group_id = g['id']
                    break
        ...
        data = {
            ...
            "phone": phone,
            "sms_phone": phone,
            "groups": [{"id": group_id}] if group_id else []
        }
        
        resp = requests.post("https://api.activetrail.com/v1/contacts", headers=headers, json=data)
        return resp.status_code in [200, 201]
    except Exception as e:
        st.error(f"שגיאה בסנכרון ל-ActiveTrail: {e}")
        return False
    ...
    with st.form("contact_form"):
        ...
        if submit_contact:
            if name and phone and email:
                ...
                # 3. ActiveTrail Sync
                sync_to_activetrail(name, phone, email)
