...
def sync_to_activetrail(name, phone, email):
    ...
    try:
        ...
        data = {
            ...
            "phone": phone,
            "sms_phone": phone,
            "status": "Subscribe",  # Essential to activate the contact for automations
            "groups": [{"id": group_id}] if group_id else []
        }
        
        resp = requests.post(f"{ACTIVETRAIL_BASE_URL}/contacts", headers=headers, json=data, timeout=10)
